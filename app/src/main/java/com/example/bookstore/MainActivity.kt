    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val launcher = rememberLauncherForActivityResult(
                ...
                val task = storage.child("test${Random.nextInt(0,11111)}$.jpg").putBytes(bitmapToByteArray(this, uri))
                task.addOnSuccessListener { uploadTask ->
                    uploadTask.metadata?.reference?.downloadUrl?.addOnCompleteListener { urlTask ->
                        saveBook(db, urlTask.result.toString())
                    }
                }
            }
            MainScreen{launcher.launch(PickVisualMediaRequest(mediaType =
...
//    decodeResource(context.resources, R.drawable.photo)
